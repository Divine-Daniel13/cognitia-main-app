...
import { CreditCard, DollarSign, TrendingUp, Download, ArrowUpRight, Search, Filter } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-10">
      <div className="flex justify-between items-center">
         ...
         <button className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold shadow-lg shadow-emerald-500/20 flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         {[
           ...
           <div key={kpi.label} className="p-8 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl">
      ...
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] overflow-hidden">
         <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
            ...
            <div className="flex space-x-3"><button className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-800"><Search size={18}/></button><button className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-800"><Filter size={18}/></button></div>
         </div>
         <div className="overflow-x-auto">
            <table className="w-full text-left">
               <thead>
                  <tr className="border-b border-slate-50 dark:border-slate-800">
                     <th className="px-8 py-6 text-[10px] font-black uppercase text-slate-400">Transaction ID</th>
                     <th className="px-6 py-6 text-[10px] font-black uppercase text-slate-400">Client Identity</th>
                     <th className="px-6 py-6 text-[10px] font-black uppercase text-slate-400">Type</th>
                     <th className="px-6 py-6 text-[10px] font-black uppercase text-slate-400">Revenue</th>
                     <th className="px-6 py-6 text-[10px] font-black uppercase text-slate-400">Status</th>
                     <th className="px-8 py-6 text-right text-[10px] font-black uppercase text-slate-400">Nexus</th>
                  </tr>
               </thead>
                  ...
                  {transactions.map((tx, idx) => (
                    <tr key={tx.id} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                       ...
                       <td className="px-6 py-5">
                          <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${tx.status === 'Success' ? 'bg-emerald-500/10 text-emerald-500' : tx.status === 'Failed' ? 'bg-rose-500/10 text-rose-500' : 'bg-amber-500/10 text-amber-500'}`}>{tx.status}</span>
                       </td>
                       <td className="px-8 py-5 text-right"><ArrowUpRight size={18} className="text-slate-300 group-hover:text-indigo-500 transition-colors cursor-pointer ml-auto"/></td>
                    </tr>
                  ))}
               </tbody>
            </table>
